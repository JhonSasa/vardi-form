...
const [recaptchaToken, setRecaptchaToken] = useState<string | null>(null)
  ...
  const res = await fetch('/actualizacion/api/check-data', {
      method: 'POST',
  ...
  useEffect(() => {
    const cargarListas = async () => {
      ...
      fetch('/actualizacion/api/sugar-list?name=sasa_tipo_documento_c&module=Contacts'),
      fetch('/actualizacion/api/sugar-list?name=sasa_tipo_solicitud_c&module=Leads'),
      fetch('/actualizacion/api/sugar-list?name=sasa_canales_autorizados_c&module=SASA_Habeas_Data'),
      ])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        const res = await fetch('/actualizacion/api/send-contact', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <main className="p-6 max-w-xl mx-auto">
      <div className="flex justify-center mb-4">
        <img src="/actualizacion/Nissan_2020_logo.svg" alt="Logo Nissan" className="h-16" />
      </div>
